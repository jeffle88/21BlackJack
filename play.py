 ...
 \ \ / / _ \| | | | | |   / _ \/ ___|| ____|   / /_   _|         |_   _\ \ 
  \ V / | | | | | | | |  | | | \___ \|  _|    | |  | |             | |  | |
   ...
   |_| \___/ \___/  |_____\___/|____/|_____|  | |  |_|    _____    |_|  | |
                                               \_\       |_____|       /_/ 
"""
	print "You lost {}chips. You currently have {}".format(player.bet, player.chips)
	if player.chips<=0: 
		print """
                              )                 
 (                         ( /(                 
...
"""
	print "You gained {}chips. You currently have {}".format(player.bet, player.chips)
def the_game():
	...
	if answer2.upper() == 'S':
		...
		if dealer.sum > 21: 
			...
			print "Dealer wins {}-{} You lost. Loser".format(dealer.sum, player.sum)
			lose()
			return
		elif dealer.sum < player.sum:
			print "You win {}-{} You're not a loser. I guess".format(player.sum, dealer.sum)
			win()
			return
		else: 
			print "You tied {}-{} That's weird".format(player.sum, dealer.sum)
			return
